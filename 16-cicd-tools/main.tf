...
resource "aws_instance" "github_runner" {
  count                  = var.github_runner ? 1 : 0
  ami                    = local.ami_id
  instance_type          = "t3.micro"
  subnet_id              = local.public_subnet_id
  vpc_security_group_ids = [local.github_runner_sg_id]
  user_data              = file("github-runner.sh")
  ...
  root_block_device {
    ...
    tags = merge(
      {
        Name = "${var.project}-${var.environment}-github-runner"
      },
      local.common_tags
    )
  }
