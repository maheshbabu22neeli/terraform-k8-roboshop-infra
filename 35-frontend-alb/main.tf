...
resource "aws_lb_listener" "https" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "<h1>Hi, I am from HTTPS Frontend ALB Fixed Response</h1>"
      status_code  = "200"
    }
  }
}
...
resource "aws_lb_target_group" "frontend" {
  ...
  port     = 80
  protocol = "HTTP"
  # if this is VM target group, then target_type should be instance.
  # if target_type is IP pods will come and register here
  target_type = "ip"
  vpc_id   = local.vpc_id
